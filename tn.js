...
const SKIP_TITLES = new Set([
  ...
  'corrigendum', 'results of tenders',
]);
...
function isValidTender(title, refNo, closingDate) {
  ...
  const hasTitle = title.length > 10 && !/^\d+$/.test(title);
  return !!(hasRef || hasDate || hasTitle);
}
...
class TnScraper extends BaseScraper {
      ...
      while (true) {
        ...
        if (pageNum === 1) {
          try {
            ...
            logger.info(`[TN] Sample row text: ${sample?.substring(0, 300)}`);
            const colCount = await this.page.$eval('table tbody tr:first-child', el => el.querySelectorAll('td').length);
            logger.info(`[TN] Columns in first row: ${colCount}`);
          } catch(e) { logger.warn(`[TN] Could not sample row: ${e.message}`); }
        }
  ...
  async _scrapePage() {
    ...
    try {
      ...
      logger.info(`[TN] Raw rows found: ${rows.length}`);
  ...
  async _extractRow(row) {
    ...
    // Auto-detect which column has date vs ref number
