...
const SEL = {
  ...
  tabs: {
    today:   'a:has-text("Closing Today")',
    week:    'a:has-text("Closing within 7 days")',
    two_weeks: 'a:has-text("Closing within 14 days")',
  },
...
const JUNK_PATTERNS = [
  ...
  /tenders\/auctions/i, /search by/i, /bid submission/i,
];
...
class TnScraper extends BaseScraper {
  ...
  async run() {
    ...
    let totalScraped = 0;
    const seenIds = new Set(); // deduplicate across tabs
    ...
    try {
      ...
      // Step 2: Click "Tenders by Closing Date" nav link
      logger.info('[TN] Navigating to Tenders by Closing Date...');
      await this.page.click(SEL.tendersByDate);
      await this.page.waitForLoadState('domcontentloaded', { timeout: 15000 });
      await this.page.waitForTimeout(2000);
      logger.info(`[TN] Current URL: ${this.page.url()}`);

      // Check for captcha
      const captchaEl = await this.page.$(SEL.captcha).catch(() => null);
      if (captchaEl) {
        throw new Error('Captcha wall appeared on Tenders by Closing Date page');
      }

      // Step 3: Scrape each tab — "Closing within 14 days" has most tenders
      const tabsToScrape = [
        { label: 'Closing within 14 days', selector: SEL.tabs.two_weeks },
        { label: 'Closing within 7 days',  selector: SEL.tabs.week },
        { label: 'Closing Today',          selector: SEL.tabs.today },
      ];

      for (const { label, selector } of tabsToScrape) {
        logger.info(`[TN] Switching to tab: ${label}`);
        try {
          ...
          logger.warn(`[TN] Could not click tab "${label}": ${err.message}`);
          continue;
        }
        ...
        while (true) {
          ...
          // Deduplicate against already seen tenders
