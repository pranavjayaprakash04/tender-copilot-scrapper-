...
function isValidTender(title, refNo, closingDate) {
  ...
  // STRICT: must have both a proper ref number AND a date, OR at minimum a ref number
  const hasRef  = refNo && REF_NO_PATTERN.test(refNo);
  const hasDate = !!closingDate && DATE_PATTERN.test(closingDate);
  // Require ref number always — this filters out all nav/form junk
  return !!(hasRef && (hasDate || title.length > 15));
}
...
class TnScraper extends BaseScraper {
  ...
  async run() {
    ...
    try {
      ...
      // Wait a bit and log current URL to detect redirects
      await this.page.waitForTimeout(3000);
      const currentUrl = this.page.url();
      logger.info(`[TN] Current URL after load: ${currentUrl}`);

      while (true) {
        logger.info(`Scraping TN page ${pageNum}`);
        await this._handleCaptchaIfPresent();

        // Log page URL each iteration
        logger.info(`[TN] Page URL: ${this.page.url()}`);
        ...
        if (pageNum === 1) {
          try {
            ...
            const sample = await this.page.$eval('table tbody tr:first-child', el => el.innerText);
            logger.info(`[TN] Total rows: ${rowCount}, Sample: ${sample?.substring(0, 200)}`);
          } catch(e) { logger.warn(`[TN] Sample failed: ${e.message}`); }
        }
  ...
  async _handleCaptchaIfPresent() {
    try {
      ...
      // After solving, wait for page to reload
      await this.page.waitForLoadState('domcontentloaded', { timeout: 15000 });
      logger.info(`[TN] After captcha URL: ${this.page.url()}`);
    } catch(e) { logger.warn(`[TN] Captcha handling error: ${e.message}`); }
  }
  ...
  async _extractRow(row) {
    ...
    const col1     = cells[1] ? await getText(cells[1]) : '';
    const col2     = cells[2] ? await getText(cells[2]) : '';
    const col3     = cells[3] ? await getText(cells[3]) : '';
